...
export class ArchiveEdit extends Component {
    ...
    editArchive = (Id) => {
        ...
        this.setState({
            ...
            live_channels: editingItem.live_channels,
            radio_channels: editingItem.radio_channels,
            archives: editingItem.archives,
            vod: editingItem.vod,
    ...
    updateHandler = (e) => {
        ...
        axios({
            method: 'PUT',
            url: API_PATH.URL + "user/favourites/" + id + '/',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer' + ticket
            },
    ...
    render() {
        ...
        const archivesItems = this.state.Archives.map(Archive => {
            return (
                <li key={Archive.id}>{Archive.live_channels}
